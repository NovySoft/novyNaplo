...
Future<List<Evals>> getAllEvals() async {
  ...
  List<Evals> tempList = List.generate(maps.length, (i) {
    ...
    temp.valueType = Description.fromJson(json.decode(maps[i]['valueType']));
    temp.kindOf = maps[i]['kindOf'];
    temp.createDate = DateTime.parse(maps[i]['createDate']).toLocal();
    //FIXME Seen date storeing
    temp.seenDate = DateTime.parse(maps[i]['seenDate']).toLocal();
    temp.mode = Description.fromJson(json.decode(maps[i]['mode']));
    ...
    temp.subject = Subject.fromJson(json.decode(maps[i]['subject']));
    ...
    var matchedEvals = allEvals.where(
      (element) {
        return (element.uid == eval.uid &&
            ...
            (element.subject.name == eval.subject.name &&
                element.uid == eval.uid &&
                element.valueType.name == eval.valueType.name);
      },
    );
